...
- **Structref-backed UDAF feature**: branch `feat/structref-udaf-stddev`, PR [#23](https://github.com/Goykhman/numbduck/pull/23). All 4 plan tasks complete. 132 tests pass locally, CI run 24541258821 pending (fixed `NRT stats are disabled` error by calling `_nrt.memsys_enable_stats()` programmatically in tests). Plan: `docs/plans/2026-04-15-structref-udaf-stddev-plan.md`.
- **NRT decref investigation (in progress)**: `context.nrt.decref()` emits `call void @NRT_decref(...)` during codegen, but the call is absent from the final LLVM IR. Verified: (1) the call IS emitted by the LLVM builder (visible in `func.basic_blocks` iteration during codegen), (2) it's gone by the time `add_ir_module` receives the module, (3) `nrtopt.remove_redundant_nrt_refct` is never called (early-exits because no `NRT_incref` in module), (4) disabling `LLVM_REFPRUNE_PASS` doesn't help, (5) the cheap LLVM optimization pass (`mpm_cheap`) or full pass (`mpm_full`) removes it. Next step: dump IR before/after `mpm_cheap.run()` to confirm which pass removes it, then check whether `NRT_decref`'s definition (from `nrtdynmod`) is linked into the module before optimization — if it's only `declare`d, LLVM may determine the call is dead. The workaround (calling `NRT_MemInfo_release` directly) works because that symbol IS defined in the C runtime and LLVM can't optimize it away.
